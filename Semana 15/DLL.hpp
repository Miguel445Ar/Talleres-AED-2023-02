...
public:
    ~DoublyLinkedList() {
        while(isEmpty() == false) popBack();
    }

    template<typename C>
    static DoublyLinkedList<C>* createFromVector(std::vector<T>& arr, 
        std::function<void(C)> show, std::function<C(T)> returnColumnValue) {
        auto size = arr.size;
        DoublyLinkedList<C>* dll = new DoublyLinkedList<C>(show);
        for(T value: arr) {
            dll->pushBack(returnColumnValue(value));
        }
        return dll;
    }
